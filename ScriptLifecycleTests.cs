...
namespace LunyScript.Test
{
	...
	public abstract class ScriptLifecycleTests : ContractTestBase
	{
		...
		public void SelfDisabled_DoesNotRunUpdates()
		{
			...
			Assert.That(gVars[nameof(LunyScript.When.Self.Steps)], Is.EqualTo(false));
			Assert.That(gVars[nameof(LunyScript.When.Self.Updates)], Is.EqualTo(false));
			Assert.That(gVars[nameof(LunyScript.When.Self.LateUpdates)], Is.EqualTo(false));
		}
		...
		public void SelfEnabled_RunsUpdates()
		{
			...
			Assert.That(gVars[nameof(LunyScript.When.Self.Steps)], Is.EqualTo(true));
			Assert.That(gVars[nameof(LunyScript.When.Self.Updates)], Is.EqualTo(true));
			Assert.That(gVars[nameof(LunyScript.When.Self.LateUpdates)], Is.EqualTo(true));
		}
	}
