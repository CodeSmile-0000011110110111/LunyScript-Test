...
namespace LunyScript.Test.Variables
{
	...
	public sealed class ComputedVariableBlockTests
	{
		...
		{
			var block = TestComputedBlock.Create(42);
			Assert.That(block.GetValue().AsDouble(), Is.EqualTo(42.0));
		}
		...
		{
			var block = TestComputedBlock.Create(42);
			Assert.That(block.GetValue<Double>(), Is.EqualTo(42.0));
		}
		...
		{
			var block = TestComputedBlock.Create(42);
			Assert.That(block.GetValue<Single>(), Is.EqualTo(42f));
		}
		...
		public void GetValueGeneric_Boolean_Returns_Correct_Value()
		{
			...
			Assert.That(block.GetValue<Boolean>(), Is.True);
		}
		...
		{
			var block = TestComputedBlock.Create(42);
			Assert.That(block.GetValue<Int32>(), Is.EqualTo(42));
		}
		...
		public void GetValueGeneric_Vector2_Returns_Correct_Value()
		{
			...
			Assert.That(block.GetValue<LunyVector2>(), Is.EqualTo(vec));
		}
		...
		public void GetValueGeneric_Vector3_Via_BaseDefault()
		{
			...
			Assert.That(block.GetValue<LunyVector3>(), Is.EqualTo(vec));
		}
		...
		{
			var block = TestComputedBlock.Create(42);
			Assert.Throws<LunyScriptVariableException>(() => block.GetValue<LunyQuaternion>());
		}
		...
		private sealed class TestVector2ComputedBlock : ComputedVariableBlock
		{
			...
			internal override Variable GetValue() => Variable.FromVector2(_value);
